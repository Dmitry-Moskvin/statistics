...
summary(fit2)
confint(fit2)# Computes confidence intervals for one or more parameters in a fitted model.


# categorical predictors
hist(swiss$Catholic, col = 'red')

swiss$religious <- as.factor(ifelse(swiss$Catholic > 60, 'Lots', 'Few'))
...
# model selection
optimal_fit <-  step(fit_full, direction = 'backward') # Select a formula-based model by AIC.
summary(optimal_fit)
